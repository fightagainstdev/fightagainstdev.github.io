...
export class ChessBoard {
    ...
    public move(prevX: number, prevY: number, newX: number, newY: number, promotedPieceType: FENChar | null): void {
        if (this._isGameOver) throw new Error("游戏结束，你可以先出");
    ...
    private isGameFinished(): boolean {
        if (this.insufficientMaterial()) {
            this._gameOverMessage = "由于不充分而平局";
            return true;
        }
        ...
        if (!this._safeSquares.size) {
            if (this._checkState.isInCheck) {
                ...
                this._gameOverMessage = prevPlayer + " 通过将死而赢";
            }
            else this._gameOverMessage = "僵局";

            return true;
        }

        if (this.threeFoldRepetitionFlag) {
            this._gameOverMessage = "由于三次重复局面而平局";
            return true;
        }

        if (this.fiftyMoveRuleCounter === 50) {
            this._gameOverMessage = "由于最多五十回合而平局";
            return true;
        }
